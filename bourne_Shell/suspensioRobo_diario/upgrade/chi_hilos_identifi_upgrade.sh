...
cd $ruta_shell
# =============================================================================
archivo="script_upgrade_hilo$HILO"
cd $ruta_shell
...
cat > $ruta_shell/$archivo.sql << eof
   ...
   rrk_upgrade_chi.rrp_identificacion(despa => $HILO,
											   pv_error => lv_error);
...
eof
   ...
   if [ $ESTADO -lt 1 ] || [ $ERROR -ge 1 ] || [ "$SALIDA" -ne "" ]; then
	  ...
	  echo "ERROR en hilo $HILO" >> detalle_hilos.log
	  echo $date >> detalle_hilos.log
      exit 1
   fi


echo "Ejecucion de hilo $HILO finalizada"
echo "EXITO en hilo $HILO" >> detalle_hilos.log
echo $date >> detalle_hilos.log
