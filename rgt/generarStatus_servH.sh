...
#funcion para asignar hilos
  local archivoSql=asigna_$TablaTrabajo.sql
  local archivoLog=$TablaTrabajo.log
  cat> $archivoSql <<EOF
  set serveroutput on
  set feed off
...
local archivoSql=actualiza_$TablaTrabajo$cont.sql
local archivoLog=$TablaTrabajo.log
local hilo=$cont
...
declare
   cursor c_base_1 is
    ...
    CURSOR C_STATUS_CUENTA(CV_CUENTA VARCHAR2 )IS
      select a.valor,b.descripcion,a.fecha_desde, a.fecha_hasta fecha_inactivacion
       ...
       and  a.id_tipo_detalle_serv in ('TAR-ESTAT','AUT-ESTAT','DPO-ESTAT','PPA-ESTAT')
       and c.codigo_doc=CV_CUENTA
       and a.valor=b.estatus
       and  a.fecha_Desde in (
                    select max(fecha_Desde)
                    from cl_detalles_servicios@axiscli_rep
                    where id_contrato =c.ID_CONTRATO
                    and  id_tipo_detalle_serv in ('TAR-ESTAT','AUT-ESTAT','DPO-ESTAT','PPA-ESTAT')
                    and c.codigo_doc=CV_CUENTA
                    )
       and rownum=1;
...
monitorSql="sqlplus -s $user/$pass @$archivoSql"
monitorSql2='SQLPLUS'
#borrado archivo temporal
#if [ -f $archivoSql ] ; then
  #rm $archivoSql
...
campo1=cuenta     #variable campo debe ser igual al campo de la tabla
